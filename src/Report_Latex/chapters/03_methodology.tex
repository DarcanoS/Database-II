...
To support low-latency analytical queries, real-time dashboards, and high-frequency ingestion cycles, the system implements a combined strategy of advanced indexing, hypertable partitioning, materialized aggregation, and execution-plan validation. This subsection consolidates the indexing design for the most critical queries in the platform and the optimization techniques applied to ensure sub-2-second p95 latency under production workloads.

\subsubsection{Indexing Strategy}

The indexing strategy was derived from the filtering patterns observed across the platform’s core analytical queries: station-based lookups, time-window filtering, pollutant comparisons, and geospatial aggregation. All indexes were validated using \texttt{EXPLAIN} and \texttt{EXPLAIN ANALYZE} to ensure alignment with the PostgreSQL query planner.

\paragraph{1. Query: Recent measurements per station}
\begin{verbatim}
SELECT *
...
\textit{Rationale:}
Matches the filter order (station → time), enabling index-only scans for dashboards querying recent data.

\paragraph{2. Query: Daily pollutant averages per city}
\begin{verbatim}
...
\textit{Rationale:}
Supports selective grouping and aggregation patterns used by dashboards and API endpoints.

\paragraph{3. Query: Time-series trend for one pollutant}
\begin{verbatim}
...
\textit{Rationale:}
Accelerates ordered retrieval and prevents sequential scans on the hypertable.

\paragraph{4. Partial index for recent data}
\begin{verbatim}
...
\textit{Rationale:}
Reduces index size and improves ingestion throughput while optimizing the most frequent queries.

\paragraph{5. BRIN index for long-term historical data}
\begin{verbatim}
...
\textit{Rationale:}
BRIN indexing minimizes disk usage and enables efficient scans across multi-year partitions.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Query Optimization Techniques}

Beyond indexing, the system integrates several additional optimization mechanisms designed for large-scale time-series workloads.

\paragraph{Hypertable Partitioning (TimescaleDB)}
The \texttt{airquality\_reading} table was partitioned into monthly time chunks with space partitioning by city. This design reduces scan ranges, improves ingestion speed, and aligns with time-bounded analytical patterns.

\paragraph{Materialized Views and Continuous Aggregates}
To avoid repeated computation of expensive rollups, the system uses:

\begin{itemize}
    \item 15-minute continuous aggregates for high-frequency dashboard refresh,
    \item hourly and daily materialized views for trend analysis,
    \item automatic backfilling of aggregates for late-arriving AQI data.
\end{itemize}

These pre-computed structures significantly reduce CPU and I/O load during peak usage.

\paragraph{Query Simplification}
Analytical queries were rewritten to avoid unnecessary subqueries and to leverage index-friendly structures, including:

\begin{itemize}
    \item replacing nested subqueries with direct aggregations,
    \item aligning WHERE clauses with index orderings,
    \item reducing join depth through pre-aggregation,
    \item offloading repetitive work into materialized or continuous aggregates.
\end{itemize}

\paragraph{Optional Caching Layer}
Frequently requested aggregates (e.g., "current Bogotá AQI", hourly PM2.5 trend) may be cached for 60–120 seconds using Redis. This eliminates redundant database access during high-traffic periods.

\paragraph{Execution Plan Validation}
All optimizations were validated using:

\begin{itemize}
    \item \texttt{EXPLAIN} and \texttt{EXPLAIN ANALYZE} to detect sequential scans or misaligned indexes,
    \item TimescaleDB execution metrics to evaluate chunk-level performance,
    \item before/after benchmarking of index creation and query rewriting.
\end{itemize}

This methodology ensured that every index or optimization produced measurable improvements in latency, I/O efficiency, and overall system throughput.
...
Given the system’s dual nature—high-frequency ingestion of time-series air-quality data and simultaneous analytical workloads—concurrency management becomes a critical aspect of the platform’s methodology. This subsection examines the primary concurrency scenarios, the risks associated with them, and the mitigation strategies adopted to maintain system stability and low-latency performance under load.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Concurrency Scenarios}

\paragraph{1. Data Ingestion vs. Analytical Queries}
The ingestion pipeline inserts new measurements every 10 minutes across multiple stations. Meanwhile, analysts and automated dashboards perform read-heavy operations on the most recent data. This creates potential contention on the main hypertable, particularly on hot partitions that receive continuous writes.

\paragraph{2. Concurrent Web Users}
Multiple end users and dashboards may query pollutant trends, AQI summaries, and forecast endpoints simultaneously. These workloads are predominantly read-heavy, increasing pressure on indexes, cached aggregates, and continuous-aggregate refresh processes.

\paragraph{3. Batch Processes Concurrent With Reads}
Periodic maintenance tasks—such as archiving old partitions, refreshing materialized views, or performing data-quality checks—may overlap with real-time read workloads. If not properly scheduled, these tasks may hold locks on hypertable chunks or indexes.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Concurrency Risks}

\begin{itemize}
    \item \textbf{Table or Chunk Contention:} Frequent inserts on the most recent time partition may collide with analytical queries scanning the same chunk, especially if missing indexes force sequential scans.
    \item \textbf{Row-Level Locking and Block Escalation:} Although TimescaleDB avoids table-level locks during inserts, exceptional analytical updates or deletes may escalate to page-level locks when processing large batches.
    \item \textbf{Deadlock Potential:} Concurrent refreshes of continuous aggregates, ingestion operations, and long-running analytical queries may attempt to access metadata tables or chunk indexes in incompatible lock modes.
    \item \textbf{Maintenance vs. Query Latency Impact:} VACUUM operations, reindexing, or chunk compression may temporarily degrade performance if executed during high-load periods.
\end{itemize}
...
\paragraph{1. Index and Query Design to Reduce Lock Duration}
Well-aligned composite indexes (e.g., \texttt{(station\_id, datetime)}, \texttt{(pollutant\_id, datetime)}) enable index-only scans, reducing lock duration and avoiding heap access. Partial indexes on recent data prevent unnecessary contention with historical partitions.

\paragraph{2. Row-Level Lock Granularity}
PostgreSQL’s MVCC model isolates readers from writers using row-level versioning, ensuring that analytical queries do not block ingestion inserts. This minimizes read-write conflicts across the system.

\paragraph{3. Strategic Hypertable Partitioning}
Monthly time partitions and additional space partitioning by city isolate write operations to recent chunks, enabling historical reads to execute concurrently without interference.

\paragraph{4. Transaction Isolation Considerations}
The system operates primarily under \texttt{READ COMMITTED}, which supports high ingestion throughput without blocking readers. Long-running reports may optionally use \texttt{REPEATABLE READ} to ensure snapshot consistency.

\paragraph{5. Scheduling and Throttling of Maintenance Jobs}
Maintenance tasks such as continuous aggregate refreshes, VACUUM/ANALYZE, and chunk compression are scheduled during periods of low user activity. TimescaleDB background workers also stagger refresh jobs to minimize lock overlap.

\paragraph{6. Optional Caching Layer}
Caching frequently requested endpoints (e.g., current AQI per city, hourly pollutant summaries) in Redis reduces query pressure on the hypertable during high concurrency periods, lowering lock contention and CPU load.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsubsection{Concurrency Validation}

Concurrency behavior was evaluated through parallel load tests simulating simultaneous ingestion and dashboard queries. Lock wait events were monitored using \texttt{pg\_stat\_activity} and \texttt{pg\_locks}, and execution latency was compared before and after index creation and caching enablement. TimescaleDB telemetry was used to identify chunk-level contention. Results show that the chosen partitioning model, indexing strategy, and read/write separation maintained stable ingestion throughput and sustained sub-2-second p95 API latency under concurrent workloads.
